...
def regular_training(model, data, labels, groups, iters, lr, weight_decay=1e-2):
    ...
    loader = DataLoader(group_data, batch_size=1000, shuffle=True)
...
def dro_training(model, data, labels, groups, iters, lr, weight_decay=1e-2):
    ...
    loader = DataLoader(group_data, batch_size=2000, shuffle=False, sampler=sampler)
    model = model.cuda()
    model.train()
